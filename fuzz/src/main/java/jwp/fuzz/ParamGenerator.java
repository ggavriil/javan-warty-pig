...
public interface ParamGenerator<T> extends AutoCloseable {
  ...
  static <T> ParamGenerator<T> suggestedFinite(Class<T> cls) {
    ...
    if (cls == Integer.class) return ofFinite(() -> Stream.concat(Stream.concat(Stream.of((Integer) null), interestingInts().boxed()), Stream.of(Integer.valueOf(121), Integer.valueOf(19))));
